...
public class EnchantJewelry {
	...
	public static enum BoltEnchantData {
		...
		SAPPHIRE(76103, 1176, 7, new int[][]
				                         {
						                         {555, 1},
						                         {564, 1}
				                         }, 9337, 9240, 50),
		EMERALD(76104, 1176, 27, new int[][]
				                         {
						                         {556, 3},
						                         {564, 1}
				                         }, 9338, 9241, 55),
		RUBY(76099, 1176, 49, new int[][]
				                      {
						                      {554, 5},
						                      {564, 1}
				                      }, 9339, 9242, 59),
		DIAMOND(76100, 1180, 57, new int[][]
				                         {
						                         {557, 10},
						                         {564, 1}
				                         }, 9340, 9243, 67),
		DRAGONSTONE(76101, 1187, 68, new int[][]
				                             {
						                             {555, 15},
						                             {557, 15},
						                             {564, 1}
				                             }, 9341, 9244, 78),
		ONYX(76102, 6003, 87, new int[][]
				                      {
						                      {557, 20},
						                      {554, 20},
						                      {564, 1}
	...
	private static void enchantEvent(final Player player, final BoltEnchantData data) {
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				for (int index = 0; index < data.getRuneRequirement().length; index++) {
					if (!ItemAssistant.hasItemAmountInInventory(player, data.getRuneRequirement()[index][0], data.getRuneRequirement()[index][1])) {
						container.stop();
						player.getPA().sendMessage(
								"You need " + data.getRuneRequirement()[index][1] + " " + ItemAssistant.getItemName(data.getRuneRequirement()[index][0]) + " to enchant this.");
						return;
					}
				}
				...
				Skilling.addSkillExperience(player, (int) data.getExp(), ServerConstants.MAGIC, false);
				...
				if (data.getRuneRequirement().length > 2) {
	...
	private static boolean hasRunes(Player c, int spellID) {
		...
		if (ens.getReq3() == 0) {
			return ItemAssistant.hasItemAmountInInventory(c, ens.getReq1(), ens.getReqAmt1()) && ItemAssistant.hasItemAmountInInventory(c, ens.getReq2(), ens.getReqAmt2())
			       && ItemAssistant.hasItemAmountInInventory(c, ens.getReq3(), ens.getReqAmt3());
		} else {
			return ItemAssistant.hasItemAmountInInventory(c, ens.getReq1(), ens.getReqAmt1()) && ItemAssistant.hasItemAmountInInventory(c, ens.getReq2(), ens.getReqAmt2());
		}
	}
			...
			if (ItemAssistant.hasItemAmountInInventory(player, enc.getUnenchanted(), 1)) {
				if (hasRunes(player, spellID)) {
					if (getEnchantmentLevel(spellID) == enc.getELevel()) {
						...
						Skilling.addSkillExperience(player, enc.getXp(), ServerConstants.MAGIC, false);
