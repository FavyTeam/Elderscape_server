...
/**
 * Total level highscores for Gladiator players.
 *
 * @author MGT Madness, created on 14-01-2018
 */
public class HighscoresTotalLevelGladiator {
	/**
	 * The class instance.
	 */
	private static final HighscoresTotalLevelGladiator instance = new HighscoresTotalLevelGladiator();
	 ...
	 */
	public final static HighscoresTotalLevelGladiator getInstance() {
		return instance;
	}

	private final static String HIGHSCORES_LOCATION = "backup/logs/highscores/total level gladiator.json";

	public HighscoresTotalLevelGladiator[] highscoresList;
	...
	public HighscoresTotalLevelGladiator() {
		this.name = "";
		this.totalLevel = 0;
		this.xp = 0;
		this.highestSkill = "";
	...
	public HighscoresTotalLevelGladiator(String username, int totalLevel, int xpInMillion, String highestSkill, String gameMode) {
		this.name = username;
	...
	public void initiateHighscoresInstance() {
		highscoresList = new HighscoresTotalLevelGladiator[ServerConstants.HIGHSCORES_PLAYERS_AMOUNT];
		for (int i = 0; i < ServerConstants.HIGHSCORES_PLAYERS_AMOUNT; i++) {
			highscoresList[i] = new HighscoresTotalLevelGladiator();
		}
		loadFile();
	}
	...
	public void loadFile() {
		...
		try (FileReader fileReader = new FileReader(file)) {
			...
			for (int i = 0; i < ServerConstants.HIGHSCORES_PLAYERS_AMOUNT; i++) {
				...
				if (!name.isEmpty()) {
					highscoresList[i] = new HighscoresTotalLevelGladiator(name, totalLevel, xp, highestSkill, gameMode);
				}
			}
		} catch (IOException e) {
			e.printStackTrace();
	...
	public void removePlayer(String name) {
		for (int index = 0; index < highscoresList.length; index++) {
			if (highscoresList[index].name.equalsIgnoreCase(name)) {
				highscoresList[index] = new HighscoresTotalLevelGladiator("", 0, 0, "", "");
			}
		}
	}
	...
	public void sortHighscores(Player player) {
		...
		if (GameType.isOsrsEco() && GameMode.getDifficulty(player, "NORMAL")) {
			return;
		}
		...
		for (int index = 0; index < highscoresList.length; index++) {
			...
			// If i am not on highscores, index 0 is the last spot on the highscores.
				...
				if (player.getTotalLevel() > highscoresList[0].totalLevel || (int) (player.getXpTotal() / 1000000) > highscoresList[0].xp) {
					highscoresList[0] = new HighscoresTotalLevelGladiator(player.getPlayerName(), player.getTotalLevel(), (int) (player.getXpTotal() / 1000000), ProfileSearch
							                                                                                                                                             .getHighestSkillXp(
		...
		for (int counter = 0; counter < highscoresList.length - 1; counter++) {
			for (int index = 0; index < highscoresList.length - 1 - counter; index++) {
				if (highscoresList[index].totalLevel > highscoresList[index + 1].totalLevel) {
					HighscoresTotalLevelGladiator temp = highscoresList[index];
