 ...
 */
public class HighscoresHybrid {
	/**
	 * The class instance.
	 */
	private static final HighscoresHybrid instance = new HighscoresHybrid();
	 ...
	 */
	public final static HighscoresHybrid getInstance() {
		return instance;
	}

	private final static String HIGHSCORES_LOCATION = "backup/logs/highscores/hybrid kills.json";

	public HighscoresHybrid[] highscoresList;
	...
	public HighscoresHybrid(String username, int playerKills, int playerDeaths, double kdr, String gameMode) {
		this.name = username;
	...
	public void initiateHighscoresInstance() {
		highscoresList = new HighscoresHybrid[ServerConstants.HIGHSCORES_PLAYERS_AMOUNT];
		for (int i = 0; i < ServerConstants.HIGHSCORES_PLAYERS_AMOUNT; i++) {
			highscoresList[i] = new HighscoresHybrid();
		}
		loadFile();
	}
	...
	public void loadFile() {
		...
		try (FileReader fileReader = new FileReader(file)) {
			...
			for (int i = 0; i < ServerConstants.HIGHSCORES_PLAYERS_AMOUNT; i++) {
				...
				if (!name.isEmpty()) {
					highscoresList[i] = new HighscoresHybrid(name, kills, deaths, kdr, gameMode);
				}
			}
		} catch (IOException e) {
			e.printStackTrace();
	...
	public void sortHighscores(Player player) {
		...
		for (int index = 0; index < highscoresList.length; index++) {
			...
			if (player.getHybridKills() > highscoresList[index].kills && highscoresList[index].name.equalsIgnoreCase(player.getPlayerName())
			    || player.deathTypes[AchievementStatistics.HYBRID] > highscoresList[index].deaths && highscoresList[index].name.equalsIgnoreCase(player.getPlayerName())) {
				highscoresList[index].kills = player.getHybridKills();
				highscoresList[index].deaths = player.deathTypes[AchievementStatistics.HYBRID];
			...
			// If i am not on highscores, index 0 is the last spot on the highscores.
				...
				if (player.getHybridKills() > highscoresList[0].kills || player.deathTypes[AchievementStatistics.HYBRID] > highscoresList[0].deaths) {
					highscoresList[0] = new HighscoresHybrid(player.getPlayerName(), player.getHybridKills(), player.deathTypes[AchievementStatistics.HYBRID],
					                                         Misc.getKDR(player.getHybridKills(), player.deathTypes[AchievementStatistics.HYBRID]), player.getGameMode());
					break;
				}
			}
		...
		for (int counter = 0; counter < highscoresList.length - 1; counter++) {
			for (int index = 0; index < highscoresList.length - 1 - counter; index++) {
				if (highscoresList[index].kills > highscoresList[index + 1].kills) {
					HighscoresHybrid temp = highscoresList[index];
