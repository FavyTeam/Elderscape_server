...
/**
 * Created by Jason MK on 2018-07-18 at 12:23 PM
 */
public class Movement {
    ...
    public static Movement create(Player player, int destX, int destY, boolean moveNear, int xLength, int yLength, int maximumPath) {
        if (destX == player.getLocalX() && destY == player.getLocalY() && !moveNear) {
            return null;
        }
        final int maximum = maximumPath;
        ...
        movement.addStep(PathFinder.localize(pathX, player.getMapRegionX()), PathFinder.localize(pathY, player.getMapRegionY()));
