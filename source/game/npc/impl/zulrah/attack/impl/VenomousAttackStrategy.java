...
import game.npc.impl.zulrah.ZulrahVenemousObject;
...
/**
 * Created by Jason MacKeigan on 2018-04-09 at 10:21 AM
 */
public class VenomousAttackStrategy implements ZulrahAttackStrategy {

	private final List<ZulrahDangerousLocation> venomousLocations;

	private ZulrahDangerousLocation currentVenomousLocation;

	public VenomousAttackStrategy(List<ZulrahDangerousLocation> venomousLocations) {
	...
	public void onCustomAttack(Entity attacker, Entity defender) {
		if (attacker.getType() == EntityType.NPC && defender.getType() == EntityType.PLAYER) {
			...
			for (Position location : currentVenomousLocation.getPositions()) {
				defenderAsPlayer.getPA().createPlayersProjectile(attackerAsNpc, location, 50, 100, 1044, 43, 31, 0, 65, 0);
			}
			attackerAsNpc.getEventHandler().addEvent(attackerAsNpc, new CycleEvent<Entity>() {
				...
				public void execute(CycleEventContainer<Entity> container) {
					...
					for (Position position : currentVenomousLocation.getPositions()) {
						ZulrahVenemousObject object = new ZulrahVenemousObject(
								11_700, position.getX(), position.getY(), defenderAsPlayer.getHeight(), 0, 10, -1, 50, false, defenderAsPlayer, attackerAsNpc);
